...
class AuthenticationsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'Can sigin registered user' do
    get login_url, headers: { 'Accept': 'application/vnd.api+json', 'HTTP_AUTHORIZATION': @auth}
    assert_response 200
  end

  test 'Does not sign in unregistered user' do
    get login_url, headers: { 'Accept': 'application/vnd.api+json', 'HTTP_AUTHORIZATION': @invalid_auth }
    assert_response 401
  end
 
  test 'Can logout user' do
    get logout_url, headers: { 'Accept': 'application/vnd.api+json', 'HTTP_AUTHORIZATION': @auth}
    assert_response 204
  end

  test 'Does not logout user with invalid token' do
    get logout_url, headers: { 'Accept': 'application/vnd.api+json', 'HTTP_AUTHORIZATION': @invalid_auth }
    assert_response 401
  end
end
